    ...
    public void CommandLineApplication(CommandLineApplication application)
    {
        ...
        application.OnExecute(() =>
        {
            if (dbmsOption.HasValue())
                Console.WriteLine(
                    $"检测到使用数据库类型-{dbmsOption.Value()}，请在生成后，只需在配置文件中，更改DbConnOptions:Url及DbType即可，支持目前主流数据库20+");
            ...
            if (csfOption.HasValue())
            {
                ...
                if (Directory.Exists(unzipDirPath)) throw new UserFriendlyException($"文件夹[{unzipDirPath}]已存在，请删除后重试");
