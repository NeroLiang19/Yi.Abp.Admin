...
public class AccountManager : DomainService, IAccountManager
{
    ...
    public async Task<string> GetTokenByUserIdAsync(Guid userId, Action<UserRoleMenuDto>? getUserInfo = null)
    {
        ...
        //判断用户状态
        if (userInfo.User.State == false) throw new UserFriendlyException(UserConst.State_Is_State);

        if (userInfo.RoleCodes.Count == 0) throw new UserFriendlyException(UserConst.No_Role);
        if (!userInfo.PermissionCodes.Any()) throw new UserFriendlyException(UserConst.No_Permission);

        if (getUserInfo is not null) getUserInfo(userInfo);
        ...
        Action<UserAggregateRoot> userAction = null)
    {
        ...
        {
            if (userAction is not null) userAction.Invoke(user);
            if (user.EncryPassword.Password == MD5Helper.SHA2Encode(password, user.EncryPassword.Salt)) return;
    ...
    public async Task UpdatePasswordAsync(Guid userId, string newPassword, string oldPassword)
    {
        ...
        if (!user.JudgePassword(oldPassword)) throw new UserFriendlyException("无效更新！原密码错误！");
    ...
    public async Task<bool> ExistAsync(string userName, Action<UserAggregateRoot> userAction = null)
    {
        ...
        if (user != null && user.UserName == userName) return true;
        return false;
    }
