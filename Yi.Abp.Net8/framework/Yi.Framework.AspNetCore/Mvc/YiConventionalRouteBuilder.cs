...
public class YiConventionalRouteBuilder : ConventionalRouteBuilder
{
        ...
        ConventionalControllerSetting configuration)
    {
        ...
        if (idParameter == null) return baseUrl;

        // 处理原始类型ID
        if (TypeHelper.IsPrimitiveExtended(idParameter.ParameterType, includeEnums: true)) return $"{baseUrl}/{{id}}";

        // 处理复杂类型ID
        var properties = idParameter.ParameterType
            .GetProperties(BindingFlags.Instance | BindingFlags.Public);

        foreach (var property in properties) baseUrl += $"/{{{NormalizeIdPropertyNameCase(property, configuration)}}}";
        ...
        ConventionalControllerSetting configuration)
    {
        ...
        if (actionNameInUrl.IsNullOrEmpty()) return baseUrl;
        ...
        if (secondaryIds.Count == 1) baseUrl += $"/{{{NormalizeSecondaryIdNameCase(secondaryIds[0], configuration)}}}";
