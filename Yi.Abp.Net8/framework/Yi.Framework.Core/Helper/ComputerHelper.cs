...
public class ComputerHelper
{
    ...
    public static CPUInfo GetCPUDetails()
    {
        ...
        if (IsUnix())
        {
            var logicalOutput = ShellHelper.Bash("lscpu | grep '^CPU(s):' | awk '{print $2}'");
            logicalProcessors = int.Parse(logicalOutput.Trim());

            var coresOutput = ShellHelper.Bash("lscpu | grep 'Core(s) per socket:' | awk '{print $4}'");
            var socketsOutput = ShellHelper.Bash("lscpu | grep 'Socket(s):' | awk '{print $2}'");
            cores = int.Parse(coresOutput.Trim()) * int.Parse(socketsOutput.Trim());
        }
        else
        {
            var output = ShellHelper.Cmd("wmic", "cpu get NumberOfCores,NumberOfLogicalProcessors /format:csv");
