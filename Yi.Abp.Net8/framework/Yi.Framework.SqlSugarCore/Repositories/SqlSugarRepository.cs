    ...
    where TEntity : class, IEntity, new()
{
    ...
    {
        if (typeof(ISoftDelete).IsAssignableFrom(typeof(TEntity)))
            return await (await GetDbSimpleClientAsync()).AsUpdateable().SetColumns(nameof(ISoftDelete.IsDeleted), true)
    ...
    public virtual async Task<bool> UpdateAsync(TEntity updateObj)
    {
        if (typeof(TEntity).IsAssignableTo<IHasConcurrencyStamp>()) //带版本号乐观锁更新
            try
            {
                var num = await (await GetDbSimpleClientAsync())
                    .Context.Updateable(updateObj).ExecuteCommandWithOptLockAsync(true);
                return num > 0;
            }
            catch (VersionExceptions ex)
            {
                throw new AbpDbConcurrencyException($"{ex.Message}[更新失败：ConcurrencyStamp不是最新版本],entityInfo：{updateObj}",
                    ex);
            }
