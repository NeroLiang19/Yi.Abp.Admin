    ...
    where TOptions : AuthenticationSchemeOptions, new()
{
    ...
    protected async Task<HttpModel> SendHttpRequestAsync<HttpModel>(string url,
        IEnumerable<KeyValuePair<string, string?>> query, HttpMethod? httpMethod = null)
    {
        ...
        else if (httpMethod == HttpMethod.Post) response = await HttpClient.PostAsync(queryUrl, null);

        var content = await response.Content.ReadAsStringAsync();
        if (!response.IsSuccessStatusCode) throw new Exception($"授权服务器请求错误,请求地址:{queryUrl},错误信息：{content}");
        VerifyErrResponse(content);
        var model = JsonConvert.DeserializeObject<HttpModel>(content);
        return model!;
    }
    ...
    {
        if (!Context.Request.Query.ContainsKey("code")) return AuthenticateResult.Fail("回调未包含code参数");
