...
public class TemplateGenManager : ITransientDependency
{
    ...
    public async Task<string> CreateTemplateAsync(TemplateGenCreateDto input)
    {
        ...
        if (!Directory.Exists(tempFileDirPath)) Directory.CreateDirectory(tempFileDirPath);
    ...
    {
        foreach (var dicEntry in dic) await ReplaceInDirectory(rootDirectory, dicEntry.Key, dicEntry.Value);
        ...
        static async Task ReplaceInDirectory(string directoryPath, string searchString, string replaceString)
        {
            ...
            // 递归遍历子目录
            var subDirectories = Directory.GetDirectories(newDirPath);
        ...
        static async Task<string> ReplaceInFiles(string directoryPath, string searchString, string replaceString)
        {
            ...
            var newDirectoryName = directoryName.Replace(searchString, replaceString);
            ...
            {
                var newFileName = file.Replace(searchString, replaceString);
                if (file != newFileName) File.Move(file, newFileName);
            }
            ...
            foreach (var file in files)
            {
                ...
                var newFileContent = fileContent.Replace(searchString, replaceString);
