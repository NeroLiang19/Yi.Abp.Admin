...
public class AccountService : ApplicationService, IAccountService
{
        ...
        string code)
    {
        ...
        {
            //成功，需要清空
            await _phoneCache.RemoveAsync(new CaptchaPhoneCacheKey(validationPhoneType, code));
            return;
        }
    ...
    public async Task PostRegisterAsync(RegisterDto input)
    {
        ...
        if (input.Phone is null) throw new UserFriendlyException("手机号不能为空");
        //临时账号
        if (input.UserName.StartsWith("ls_")) throw new UserFriendlyException("注册账号不能以ls_字符开头");
    ...
    public async Task<UserRoleMenuDto> GetAsync()
    {
        ...
        if (userId is null) throw new UserFriendlyException("用户未登录");
        ...
        PhoneCaptchaImageDto input)
    {
        ...
        //防止暴刷
        if (value is not null) throw new UserFriendlyException($"{input.Phone}已发送过验证码，10分钟后可重试");
    ...
    public async Task<string> PostRetrievePasswordAsync(RetrievePasswordDto input)
    {
        ...
        if (entity is null) throw new UserFriendlyException("该手机号码未注册");
    ...
    {
        var userId = _currentUser.Id;
        if (_currentUser.Id is null) throw new AbpAuthorizationException("用户未登录");
        ...
        if (routerType is null || routerType == "ruoyi")
            ...
            output =
                ObjectMapper
                    .Map<List<MenuDto>, List<MenuAggregateRoot>>(menus.Where(x => x.MenuSource == MenuSourceEnum.Ruoyi)
                        .ToList()).Vue3RuoYiRouterBuild();
        else if (routerType == "pure")
            //将后端菜单转换成前端路由，组件级别需要过滤
            output =
                ObjectMapper
                    .Map<List<MenuDto>, List<MenuAggregateRoot>>(menus.Where(x => x.MenuSource == MenuSourceEnum.Pure)
                        .ToList()).Vue3PureRouterBuild();
    ...
    {
        var userId = input.UserId == null ? _currentUser.GetId() : input.UserId.Value;

        var entity = await _userRepository.GetByIdAsync(userId);

        if (entity.Icon == input.Icon) return false;
