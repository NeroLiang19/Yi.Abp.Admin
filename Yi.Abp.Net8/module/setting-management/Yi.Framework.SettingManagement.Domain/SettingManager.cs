...
public class SettingManager : ISettingManager, ISingletonDependency
{
        ...
        bool fallback = true)
    {
        ...
        if (!providerList.Any()) return new List<SettingValue>();
        ...
        foreach (var setting in settingDefinitions)
        {
            ...
            if (setting.IsInherited)
                foreach (var provider in providerList)
                {
                    ...
                    if (providerValue != null) value = providerValue;
                }
            else
            ...
            if (setting.IsEncrypted) value = SettingEncryptionService.Decrypt(setting, value);

            if (value != null) settingValues[setting.Name] = new SettingValue(setting.Name, value);
        }
        ...
        bool forceToSet = false)
    {
        ...
        if (setting.IsEncrypted) value = SettingEncryptionService.Encrypt(setting, value);
        ...
        bool fallback = true)
    {
        ...
        if (!fallback || !setting.IsInherited) providers = providers.TakeWhile(c => c.Name == providerName);
