    ...
    IAuthService
{
    ...
    private readonly ISqlSugarRepository<AuthAggregateRoot, Guid> _repository;
    ...
    {
        var (openId, _) = await GetOpenIdAndNameAsync(scheme);
        var authEntity = await _repository.GetAsync(x => x.OpenId == openId && x.AuthType == scheme);

        if (authEntity is null) throw new UserFriendlyException("第三方登录失败，请先注册后，在个人中心进行绑定该第三方后使用");
    ...
    {
        var (openId, name) = await GetOpenIdAndNameAsync(scheme);
        var userId = CurrentUser.Id;
        var authEntityAny = await _repository.IsAnyAsync(x => x.OpenId == openId && x.AuthType == scheme);
        if (authEntityAny) throw new UserFriendlyException("绑定失败，该第三方账号已被注册");
    ...
    {
        if (HttpContext is null) throw new AggregateException("HttpContext 参数为空");
        var authenticateResult = await HttpContext.AuthenticateAsync(scheme);
        if (!authenticateResult.Succeeded) throw new UserFriendlyException(authenticateResult.Failure.Message);
    ...
    protected override async Task CheckCreateInputDtoAsync(AuthCreateOrUpdateInputDto input)
    {
        ...
        if (isAny) throw new UserFriendlyException("该用户已有此应用授权");
    }
}