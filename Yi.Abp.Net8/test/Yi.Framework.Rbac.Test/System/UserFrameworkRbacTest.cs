...
public class UserFrameworkRbacTest : YiFrameworkRbacTestBase
{
    ...
    {
        var createdUser = await _userService.CreateAsync(new UserCreateInputVo
            { Nick = "nickTest", Sex = SexEnum.Woman, UserName = "UpdateUserTest", Password = "654321" });
        await _userService.UpdateAsync(createdUser.Id,
            new UserUpdateInputVo
                { Nick = "nickTest2", Sex = SexEnum.Woman, UserName = "UpdateUserTest", Password = "123456888abc" });
    ...
    {
        var createdUser = await _userService.CreateAsync(new UserCreateInputVo
            { UserName = "DeleteUserTest", Password = "123456" });
