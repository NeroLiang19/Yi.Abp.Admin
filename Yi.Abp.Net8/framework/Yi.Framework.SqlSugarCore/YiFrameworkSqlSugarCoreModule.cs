...
public class YiFrameworkSqlSugarCoreModule : AbpModule
{
    ...
    private void ConfigureDbOptions(IServiceCollection services, IConfiguration configuration)
    {
        ...
        // 配置默认连接字符串
        Configure<AbpDbConnectionOptions>(options => { options.ConnectionStrings.Default = dbConnOptions.Url; });
    ...
    private void ConfigureGuidGenerator(IServiceCollection services)
    {
        ...
        Configure<AbpSequentialGuidGeneratorOptions>(options => { options.DefaultSequentialGuidType = guidType; });
    }
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        // 初始化数据库
        if (options.EnabledCodeFirst) await InitializeDatabase(serviceProvider);

        // 初始化种子数据
        if (options.EnabledDbSeed) await InitializeSeedData(serviceProvider);
    }
    ...
    private async Task InitializeDatabase(IServiceProvider serviceProvider)
    {
        ...
        if (entityTypes.Any()) db.CopyNew().CodeFirst.InitTables(entityTypes.ToArray());
    }
