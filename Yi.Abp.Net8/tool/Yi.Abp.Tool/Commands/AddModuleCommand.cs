...
public class AddModuleCommand : ICommand
{
    ...
    public void CommandLineApplication(CommandLineApplication application)
    {
        ...
        application.OnExecute(() =>
        {
            ...
            if (modulePathOption.HasValue()) modulePath = modulePathOption.Value();


            //解决方案默认在模块文件夹上一级，也可以通过s进行指定
            var slnPath = "../";

            if (solutionOption.HasValue()) slnPath = solutionOption.Value();
            ...
            var cmdCommands = dotnetSlnCommandPart.Select(x =>
                $"dotnet sln \"{slnPath}\" add \"{Path.Combine(modulePath, $"{moduleName}.{x}")}\"").ToArray();
    ...
    private string CheckFirstSlnPath(string slnPath)
    {
        ...
        if (slnFiles.Length > 1) throw new UserFriendlyException("当前目录包含多个sln解决方案，请只保留一个");
        if (slnFiles.Length == 0) throw new UserFriendlyException("当前目录未找到sln解决方案，请检查");
