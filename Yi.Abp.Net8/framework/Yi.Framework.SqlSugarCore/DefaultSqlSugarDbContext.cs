...
public class DefaultSqlSugarDbContext : SqlSugarDbContext
{
    ...
    public override void OnLogExecuted(string sql, SugarParameter[] pars)
    {
        if (DbOptions.EnabledSqlLog)
        {
            ...
            LoggerFactory.CreateLogger<SqlSugarDbContext>().LogDebug(sqllog);
        }
    }
    ...
    public override void EntityService(PropertyInfo propertyInfo, EntityColumnInfo entityColumnInfo)
    {
        ...
        if (propertyInfo.PropertyType == typeof(ExtraPropertyDictionary)) entityColumnInfo.IsIgnore = true;

        if (propertyInfo.Name == nameof(Entity<object>.Id)) entityColumnInfo.IsPrimarykey = true;
    }
    ...
    ///     工作单元管理器
    /// </summary>
    protected IUnitOfWorkManager UnitOfWorkManagerService =>
    ...
    private void HandleInsertAuditFields(object oldValue, DataFilterModel entityInfo)
    {
        ...
        {
            if (DateTime.MinValue.Equals(oldValue)) entityInfo.SetValue(DateTime.Now);
        }
        ...
        {
            if (CurrentTenantService.Id is not null) entityInfo.SetValue(CurrentTenantService.Id);
        }
    }
    ...
    protected virtual EntityEventReport? CreateEventReport(object entity)
    {
        ...
        if (generatesDomainEventsEntity == null) return eventReport;
