...
public class YiFrameworkRbacTestModule : AbpModule
{
    ...
    public override async Task OnPostApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        var defaultRoleEntity =
            await roleRep._DbQueryable.Where(x => x.RoleCode == UserConst.DefaultRoleCode).FirstAsync();
        var menuIds = await menuRep._DbQueryable.Where(x => x.PermissionCode.Contains("user")).Select(x => x.Id)
            .ToListAsync();
